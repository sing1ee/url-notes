...
## [(106) 独立开发者申请英国公司开通stripe一条龙教程 - YouTube](https://www.youtube.com/watch?v=MPTbBPH7xLM)
...
## [Stacklist | The easiest way to save, organize, share, and discover your favorite things.](https://www.stacklist.app/?ref=producthunt)
...
## [gsidhu/buzee-tauri：一款超快速全文搜索应用 --- gsidhu/buzee-tauri: A superfast full-text search application](https://github.com/gsidhu/buzee-tauri)
...
## [yenche123/liubai: Supercharge yourself!](https://github.com/yenche123/liubai)
...
- Note:  
Shortest 是一个基于 AI 的自然语言端到端测试框架。它建立在 Playwright 之上,提供了自然语言测试执行、GitHub 集成和 2FA 支持等功能。

### 主要功能点
- 基于自然语言的端到端测试框架
- 使用 Anthropic Claude API 进行 AI 驱动的测试执行
- 基于 Playwright 构建
- 支持 GitHub 集成和 2FA 登录
...
## [federicotorrielli/BetterWhisperX: Better WhisperX: Automatic Speech Recognition with Word-level Timestamps (& Diarization)](https://github.com/federicotorrielli/BetterWhisperX)
- Time: 2024/12/29 20:04:49
- Note:  
BetterWhisperX 是一个基于 WhisperX 的改进版本,提供了更快的自动语音识别(ASR)功能,包括单词级时间戳和说话人分离。它解决了原版 WhisperX 中的一些问题,并增加了新的功能。

### 主要功能点
- 使用 faster-whisper 后端,可以在 8GB 以下的 GPU 上运行 whisper large-v2 模型
- 使用 wav2vec2 对齐模型实现更准确的单词级时间戳
- 支持多说话人 ASR,使用 pyannote-audio 进行说话人分离
- 使用语音活动检测(VAD)预处理,减少语音幻觉并支持批处理而不会降低识别准确率
...
## [Card Components | 21st.dev - The NPM for Design Engineers](https://21st.dev/s/card)
- Time: 2024/12/29 20:41:22
- Note: 前端组件，shadcn 可以用，类似

## [ZGGSONG/STranslate: A ready-to-go translation ocr tool developed by WPF/WPF 开发的一款即用即走的翻译、OCR工具](https://github.com/ZGGSONG/STranslate/)
- Time: 2024/12/29 20:47:48
- Note:  
STranslate是一款基于WPF开发的即用即走的翻译和OCR工具。随着免费服务的更新和局限性,建议申请服务并进行相应的配置以获得更好的用户体验。
...
## [量化投资的毫秒战争](https://mp.weixin.qq.com/s/LBx9uOhCovp6iBUHmW_XTQ)
- Time: 2024/12/29 20:49:25
- Note: 量化投资的全面介绍
...
- Time: 2024/12/29 21:39:13
- Note:  
Roo-Cline 是 Cline 的一个分支项目,增加了一些实验性功能。它主要是自己在编写代码,偶尔会有一些人工指导。

### 主要功能点
- 支持拖放图像到聊天中
- 增强提示按钮(仅限于 OpenRouter 模型)
- 添加声音反馈效果
- 支持使用更大的 1280x800 浏览器窗口
- 从历史记录快速复制提示
- 支持 OpenRouter 压缩
- 在系统提示中包含当前时间
- 使用文件系统监视器更可靠地监视文件系统更改
- 支持多种语言(英语、日语、西班牙语、法语、德语等)
- 支持 Meta 3、3.1 和 3.2 模型(通过 AWS Bedrock)
- 每个工具的 MCP 自动批准
- 启用/禁用 MCP 服务器
- 自动写入后的可配置延迟,以允许诊断检测潜在问题
- 与原始 Cline 并行运行
...
- Time: 2024/12/30 10:49:45
- Note: AI量化

## [Learn Any Language with MyLens AI | Free, Fun, and Personalized](https://mylens.ai/apps/easylang)
- Time: 2024/12/30 11:30:14
- Note: AI驱动的结构化学外语

## [量化前沿｜使用大语言模型揭露企业年报中掩盖的坏消息](https://mp.weixin.qq.com/s/6wxY7LW3ok1GiVSEo0jScw)
- Time: 2024/12/30 15:00:55
- Note: 使用大语言模型揭露企业年报中掩盖的坏消息

## [[文献精汇] 使用 Python 的 VWAP TWAP 和 PoV 实现利润最大化](https://mp.weixin.qq.com/s/XwvNnxKLYgJzaGABGCF_EQ)
- Time: 2024/12/30 15:01:06
- Note: 使用 Python 的 VWAP TWAP 和 PoV 实现利润最大化

## [你将如何设计剧情，Meta-Prompting：LLM分支叙事WHAT-IF叙事生成重大突破 |最新](https://mp.weixin.qq.com/s/h8WjzQgdGAM47lUhBPTKRw)
- Time: 2024/12/30 15:01:29
- Note: 小说 prompt 设计

## [国外指标：一目均衡表Ichimoku动量突破指标，多空量化云带](https://mp.weixin.qq.com/s/Lk3iW9oZEtRnj8BWYQTfkQ)
- Time: 2024/12/30 15:01:56
- Note: 一目均衡指标
...
- Time: 2024/12/30 17:20:38
- Note: 和我开发的系统一样

## [siyuan-note/siyuan: A privacy-first, self-hosted, fully open source personal knowledge management software, written in typescript and golang.](https://github.com/siyuan-note/siyuan)
- Time: 2024/12/30 17:20:59
- Note: 基于 AI 的日记管理系统，我也开发了一个思路类似
...
- Time: 2024/12/30 17:21:34
- Note:  
Phidata 是一个用于构建多模态智能代理的框架。它允许开发者创建具有记忆、知识、工具和推理能力的智能代理,并提供了一个漂亮的代理用户界面进行交互。
...
- Python
- OpenAI模型
- 多种外部工具集成(如DuckDuckGo, YFinance等)
- 向量数据库(如LanceDB)
- 文件存储(如PDF)
...
- Time: 2024/12/30 17:22:16
- Note: 这是一个为 iTerm/iTerm2 提供超过 325 种终端配色方案/主题的项目。它还包括了对 Terminal、Konsole、PuTTY、Xresources、XRDB、Remmina、Termite、XFCE、Tilda、FreeBSD VT、Terminator、Kitty、MobaXterm、LXTerminal、Microsoft's Windows Terminal、Visual Studio、Alacritty 等其他终端模拟器的支持。
...
- Time: 2024/12/30 19:56:11
- Note: ImHex 是一款面向逆向工程师、程序员和深夜工作时注重视力保护的人群的十六进制编辑器。它提供了丰富的功能,包括自定义的 C++ 风格的模式语言、主题支持、数据导入导出、数据分析和可视化、YARA 规则支持等。

## [elizaOS/eliza: Autonomous agents for everyone](https://github.com/elizaOS/eliza?tab=readme-ov-file)
- Time: 2024/12/30 19:58:12
- Note: Eliza 是一个功能强大的自主代理系统,支持多种聊天平台(Discord、Twitter、Telegram等),并可与各种语言模型(Llama、OpenAI、Anthropic等)集成。它具有丰富的功能,包括多代理和房间支持、文档存储和检索、可扩展性强等。
...
- Time: 2024/12/30 19:59:21
- Note: RetroArch 是一个跨平台的高级前端，用于 libretro API。它是一个开源的模拟器前端,支持各种游戏系统的模拟。

## [cobalt](https://cobalt.tools/?ref=producthunt)
- Time: 2024/12/30 20:03:33
- Note: 在线视频解析链接，然后下载
...
- Time: 2024/12/30 20:05:18
- Note: KAG是一个基于OpenSPG引擎和大型语言模型的逻辑推理和问答框架,用于为垂直领域知识库构建逻辑推理和问答解决方案。它可以有效地克服传统RAG向量相似性计算的模糊性和OpenIE引入的噪音问题。

## [一种高胜率的交易系统：均值回归交易策略实例详解](https://mp.weixin.qq.com/s?__biz=MjM5MDQ5OTMxMA==&mid=2649297638&idx=1&sn=473583f9092601f963d4dc105216e17d&scene=21#wechat_redirect)
- Time: 2024/12/30 20:53:06
- Note: 布林带均值回归交易策略    现在让我们举例说明可以在市场中使用的实际均值回归策略。在这个策略中，我们将布林带作为波动率均值回归设置信号。我们介绍的策略是由资深交易员乔·罗斯( Joe Ross ) 首次提出的，该策略称为 Gimmee bar 交易策略。    从本质上讲，Gimmee bar 交易策略被用于盘整区间顶部寻找做空机会，或是盘整区间底部寻找做多机会。当价格处于比较小的波动区间，而且很少突破通道时，这种策略就会取得较好的表现。    做多交易信号规则：    价格应该在出区间波动，以及布林带上线轨之间相对较远； 价格必须触及布林带的下轨； 等待第一个上涨烛台触及布林带下轨后。下一根看涨烛台被称为 Gimmee bar； 在 Gimmee bar 上方入场交易； 在 Gimmee bar 下方设置止损； 当价格接近或触及布林带上轨时退出交易。  做空交易信号规则：  价格应该在出区间波动，以及布林带上线轨相距较远； 价格必须触及布林带上轨； 等待第一个烛台触及布林带上轨后，下一根看跌烛台被称为 Gimmee bar； 在 Gimmee bar 下方入场交易； 在 Gimmee bar 上方设置止损； 当价格接近或触及布林带下轨时退出交易。

## [jwmao1/story-adapter: A Training-free Iterative Framework for Long Story Visualization](https://github.com/jwmao1/story-adapter)
- Time: 2024/12/31 09:57:45
- Note: 该项目提出了一种名为"Story-Adapter"的训练自由和计算高效的长故事可视化框架。该框架采用迭代的方式,利用前一轮生成的所有图像作为全局参考,通过跨注意力机制来保持语义一致性,并逐步优化图像生成,从而产生更精细和细节的交互效果,特别适用于长故事场景。
...
- Time: 2024/12/31 10:02:36
- Note: 这是一个自动视频生成器项目。用户输入主题文字，系统会自动生成解说视频。它包括以下主要功能:

## [TrueMyst/BeatPrints: ☕ BeatPrints，轻松创建引人注目的 Pinterest 风格音乐海报。 --- TrueMyst/BeatPrints: ☕ BeatPrints, create eye-catching, Pinterest-style music posters effortlessly.](https://github.com/TrueMyst/BeatPrints)
- Time: 2024/12/31 14:36:43
- Note: BeatPrints 是一个免费的 Python 项目,可以帮助用户轻松创建漂亮的音乐海报。它集成了 Spotify 和 LRClib API,可以为您最喜欢的歌曲或专辑设计定制海报。

## [binance/binance-connector-python: Simple connector to Binance Public API](https://github.com/binance/binance-connector-python)
- Time: 2024/12/31 15:41:16
- Note: 币安现货 api

## [[8000字讲解]人人都能学会的英文网站 Adsense 赚钱入门](https://mp.weixin.qq.com/s?__biz=MjM5OTIzMzYyMA==&mid=2650079316&idx=1&sn=02cd11a4bee177343b05e6798913159b&chksm=bf3f316f8848b879f22de143965d98c5094220f72e38808741627173d0fea40e9182c1e1da73&scene=21#wechat_redirect)
- Time: 2024/12/31 17:38:19
- Note: 现在我的思路是，我们开发工具需要花费时间，但是开发一个页面可能只需要一两个小时。因此，我首先制作了一个页面，然后按照层级结构完善页面，并让搜索引擎收录。在此基础上，如果我能获得流量，我就会回过头来开发一个工具。
